...
class UserRepo(db: WishlistDatabase) {
    ...
    suspend fun addUser(name: String, email: String) = dbQuery {
    ...
    suspend fun deleteUser(id: String) = dbQuery { userQueries.deleteUserById(id) }


    suspend fun getUserById(id: String): User_entity = dbQuery { userQueries.findUserById(id).executeAsOne() }

    suspend fun getUsersByIds(ids: List<String>): List<User_entity> = dbQuery {
