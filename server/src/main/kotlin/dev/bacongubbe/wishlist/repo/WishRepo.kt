    ...
    suspend fun addWish(userId : String, request: AddWishRequest) : Wish_entity =
        dbQuery {
            wishlistQueries.transactionWithResult {
                val wishlist = wishlistQueries.getListByIdAndOwner(request.wishlistId, userId).executeAsOneOrNull()?.let { wishlist ->
                    val result = wishQueries.insertWish(wishlist.id, request.name, request.description, request.links).executeAsOne()
                    wishQueries.getWishById(result).executeAsOne()
                }
                wishlist ?: throw NoSuchElementException("wishlist ${request.wishlistId} not found, or you are not it's owner")
            }
        }
